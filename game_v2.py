...
class statistics:
    ...
    def add_update_value(self, key, value):
    ...
    def add_update_array(self, key, value):
        if key in self.settings:
            if type(self.settings[key]) == np.ndarray:
                self.settings[key] = np.append(self.settings[key], value)
        else:
...
def play_game(agent, episodes=5000):
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            stats.add_update_array("ai_time", time.time_ns() - time_ai)
...
def human_vs_ai(agent):
    ...
    while True:
        ...
        if game.is_full():
            print("It's a draw!")
            stats.add_update_value("withdrawn", 1)
            stats.add_update_array("game_play", "Withdrawn")
        ...
        if game.current_winner:
            ...
            if game.current_winner == human_player:
                ...
                stats.add_update_value("won_human", 1)
                stats.add_update_array("game_play", "human")
                ...
                stats.add_update_value("won_ai", 1)
                stats.add_update_array("game_play", "ai")
